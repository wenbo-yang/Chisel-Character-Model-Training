...
class CharacterModelTrainingServiceConfig(ModelTrainingBaseConfig): 
    def __init__(self, env = None):
        ...
        self.storage_url = [x for x in self.__service_config["storage"] if x["env"] == self.env][0]["url"]
        self.temp_image_path = [x for x in self.__service_config["tempImagePath"] if x["env"] == self.env][0]["url"]
